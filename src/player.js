...
import {
  ...
  PLAYER_W, PLAYER_H, MAX_CHARGE, ABSORB_RATE, DISCHARGE_RATE, C,
...
export class Player {
  ...
  _applyPhysics(dt, level) {
    ...
    // Drop-through one-way (hold down + jump)
    const dropDown = Input.heldAny('ArrowDown', 'KeyS') &&
                     Input.pressedAny('ArrowDown', 'KeyS');
